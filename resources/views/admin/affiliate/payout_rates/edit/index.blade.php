...
<form 
    method="POST" 
    action="{{ LaravelLocalization::localizeUrl("/admin/affiliate/payout-rates/edit/{$rate->id}") }}" 
    ...
    <div class="row">
        ...
        <div class="col-md-8 col-lg-9">
            ...
            <div class="card">
                <div class="card-body">
                    <div class="card-heading pb-3 mb-3">{{ __('lang.edit_country') }}</div>
                    <div class="row">
                        <div class="col-sm-6 col-md-12 col-lg-6 mb-4 mb-lg-0 mb-md-4 mb-sm-0">
                            ...
                            <select class="form-select" name="country" id="country" aria-label="Country">
                                ...
                                @foreach ($countries as $key => $name)
                                    <option value="{{ "{$key},{$name}" }}"{{ "{$key},{$name}" == "{$rate->country_code},{$rate->country_name}" ? ' selected' : '' }}>{{ $name }}</option> 
                                @endforeach
                            </select>
                        </div>
                        <div class="col-sm-6 col-md-12 col-lg-6">
                            ...
                            <input type="number" name="rate" class="form-control" id="rate" 
                                value="{{ $rate->rate }}" min="0" max="1000" step="0.0001" autocomplete="off">
                        </div>
                    </div>
                </div>
            </div>
