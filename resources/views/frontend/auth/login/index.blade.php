...
@section("content")
    <form 
        action="{{ LaravelLocalization::localizeUrl('/login') }}" 
        method="POST" 
        class="width-sm py-5 m-auto animate__animated{{ $errors->any() || session('error') ? ' animate__shakeX' : ' animate__zoomIn' }}">
        <div class="card">
            <div class="card-body">
                <div class="text-center mb-4">
                    <h1 class="auth-title">{{ __('lang.login_account') }}</h1>
                    <p class="auth-subtitle m-0">{{ __('lang.login_account_subtitle') }}</p>
                </div>
                @if (setting('go_status') == 1)
                    <div class="social-auth">
                        <a 
                            href="{{ LaravelLocalization::localizeUrl('/login/google/redirect') }}" 
                            ...
                            <img 
                                ...
                                @if (setting('lazyload') == 1)
                                    ...
                                    data-src="{{ url("assets/image/google_logo.svg") }}">
                                @else
                                    src="{{ url("assets/image/google_logo.svg") }}">
                                @endif
                            <span>{{ __('lang.signin_google') }}</span>
                        </a>
                    </div>
                    <div class="auth-or d-flex my-4">
                        <hr class="my-auto flex-grow-1">
                        <div class="px-4">{{ __('lang.or') }}</div>
                        <hr class="my-auto flex-grow-1">
                    </div>
                @endif
                @if ($errors->any())
                ...
                <div class="mb-4">
                    <label for="password" class="form-label d-flex">
                        {{ __('lang.password') }}
                        <a 
                            href="{{ LaravelLocalization::localizeUrl(pageSlug('forgot_password', true)) }}" 
                            class="forgot my-auto ms-auto">
                ...
                <div>
                    <button type="submit" class="btn btn-color-1 w-100">
                        <span class="text-uppercase">{{ __('lang.sign_in') }}</span>
                ...
                <p class="auth-bottom text-md text-center mt-4 mb-0">
                    {{ __('lang.no_account') }} <a href="{{ LaravelLocalization::localizeUrl(pageSlug('register', true)) }}">{{ __('lang.sign_up') }}</a>
                </p>
                @csrf
            </div>
        </div>
