...
@section("content")
    ...
    @endif
    <form 
        action="{{ LaravelLocalization::localizeUrl('/forgot-password') }}" 
        method="POST" 
        class="width-sm py-5 m-auto animate__animated{{ $errors->any() || session('error') ? ' animate__shakeX' : ' animate__zoomIn' }}">
        <div class="card">
            <div class="card-body">
                <div class="text-center mb-4">
                    <h1 class="auth-title">{{ __('lang.reset_your_password') }}</h1>
                    <p class="auth-subtitle m-0">{{ __('lang.send_email_text') }}</p>
                </div>
                @if ($errors->any())
                ...
                <div>
                    <button type="submit" class="btn btn-color-1 w-100">
                        <span class="text-uppercase">{{ __('lang.reset_password') }}</span>
                ...
                <p class="auth-bottom text-md text-center mt-4 mb-0">
                    {{ __('lang.return') }} <a href="{{ LaravelLocalization::localizeUrl(pageSlug('login', true)) }}">{{ __('lang.login') }}</a>
                </p>
                @csrf
            </div>
        </div>
