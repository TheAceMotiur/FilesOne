<section class="page-header">
    <div 
        ...
        style="background: url({{ img('page', widget('blog','header','bg_image'), 'lg') }});">
        <div class="mask"></div>
        <div class="small-container container text-center mx-auto">
            @if (widget('blog','header','upper_title'))
                <p class="upper-title mb-1">
                    {{ widget('blog','header','upper_title') }}
                </p>
            @endif
            @if (widget('blog','header','title'))
                <h1 class="title position-relative mb-3">
                    {{ widget('blog','header','title') }}
                </h1>
            @endif
            @if (widget('blog','header','upper_title') || widget('blog','header','title'))
                <img 
